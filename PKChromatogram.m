    ...
    for (i = 1; i < numberOfPoints; i++) {
        if (totalIntensity[i]/[self baselineValueAtScan:i] > (1.0 + peakIdentificationThresholdF)){
            ...
            if ((top != start && top != end) && ((totalIntensity[top] - [self baselineValueAtScan:top])/maximumIntensity > peakIdentificationThresholdF)) { // Sanity check
                PKPeak *newPeak = [self peakFromScan:start toScan:end];
...
- (PKPeak *)peakFromScan:(int)startScan toScan:(int)endScan {
    ...
    PKPeak *newPeak = [[PKPeak alloc] init];
...
- (BOOL)combinePeaks:(NSArray *)peaksToCombine {
    ...
    for (i = 1; i < peakCount; i++) {
    	...
    	if ([peak start] < [(PKPeak *)[peaksToCombine objectAtIndex:indexForStart] start]) {
            indexForStart = i;
        }
    ...
    [peak setStart:[(PKPeak *)[peaksToCombine objectAtIndex:indexForStart] start]];
...
- (void)removeObjectFromPeaksAtIndex:(int)index{
	...
	PKPeak *aPeak = [peaks objectAtIndex:index];
    if (!aPeak) {
...
- (void)replaceObjectInPeaksAtIndex:(int)index withObject:(PKPeak *)aPeak{
	PKPeak *replacedPeak = [peaks objectAtIndex:index];
