	...
	if (!_didDrag) {
		if ([theEvent clickCount] == 1) { // Single click
            ...
            if (([theEvent modifierFlags] & NSControlKeyMask) && ([theEvent modifierFlags] & NSAlternateKeyMask)) {
                // add baseline point
                PKChromatogram *chromatogram = [self chromatogramAtPoint:mouseLocation];
...
//				JKLogDebug(@"add peak");
		...
		if (([theEvent modifierFlags] & NSCommandKeyMask) && ([theEvent modifierFlags] & NSAlternateKeyMask)) {
            ...
            PKPeak *newPeak = [theChromatogram peakFromScan:[self scanAtPoint:_mouseDownAtPoint] toScan:[self scanAtPoint:mouseLocation]];
