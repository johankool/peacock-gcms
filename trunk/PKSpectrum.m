...
{
    int maximumMass = lroundf(pk_stats_float_max([self masses],[self numberOfPoints]));
...
- (PKGCMSDocument *)document {
    return (PKGCMSDocument *)[super document];
}
...
//}
- (float)maxIntensity {
    return pk_stats_float_max(intensities, numberOfPoints);
}

- (PKSpectrum *)spectrumBySubtractingSpectrum:(PKSpectrum *)inSpectrum {
	int i,j,k,count1,count2;
	float d1, d2, d3, d4;
	PKSpectrum *outSpectrum = [[PKSpectrum alloc] init];
...
- (PKSpectrum *)normalizedSpectrum {
    ...
    float maximumIntensity = pk_stats_float_max(intensities,numberOfPoints);
...
- (float)scoreComparedTo:(id <PKComparableProtocol>)comparableObject { 
    return [self scoreComparedTo:comparableObject usingMethod:[(PKGCMSDocument *)[self document] scoreBasis] penalizingForRententionIndex:[(PKGCMSDocument *)[self document] penalizeForRetentionIndex]];
}
...
- (float)oldScoreComparedTo:(id <PKComparableProtocol>)libraryEntry usingMethod:(int)scoreBasis penalizingForRententionIndex:(BOOL)penalizeForRetentionIndex { // Could be changed to id <protocol> to resolve warning	
	...
	score2 = 0.0f;
	score3 = 0.0f;
	maxIntensityLibraryEntry = pk_stats_float_max([libraryEntry intensities],[libraryEntry numberOfPoints]); // use correct count!!!!! 
    ...
    if ([self document]) {
        minScannedMassRange = [[(PKGCMSDocument *)[self document] minimumScannedMassRange] floatValue];
        maxScannedMassRange = [[(PKGCMSDocument *)[self document] maximumScannedMassRange] floatValue];        
...
- (float)minScannedMassRange {
    return [[(PKGCMSDocument *)[self document] minimumScannedMassRange] floatValue];
}
- (float)maxScannedMassRange {
    return [[(PKGCMSDocument *)[self document] maximumScannedMassRange] floatValue];
}
