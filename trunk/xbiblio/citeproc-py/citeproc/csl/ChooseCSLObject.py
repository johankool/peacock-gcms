...
class ChooseCSLObject(ParentCSLObject):
    def __init__(self, attrs):
        super(ChooseCSLObject, self).__init__(attrs)
    
    def formatted_text(self, reference, options):
        for child in self.children:
            if child.evaluate(reference, options):
                return child.formatted_text(reference, options)
        return ""
...
class ChooseCSLObjectTests(unittest.TestCase):
    def setUp(self):
        pass
