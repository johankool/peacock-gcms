...
class InfoCategoryCSLObject:
    ...
    def xml(self):
        ...
        if self.scheme <> "":
            attributes += " scheme=\"%s\"" % (quoteattr(self.scheme),)
        if self.label <> "":
            attributes += " label=\"%s\"" % (quoteattr(self.label),)
        return "<category%s/>\n" % (attributes,)


class InfoCategoryCSLObjectTests(unittest.TestCase):
    def setUp(self):
        pass
