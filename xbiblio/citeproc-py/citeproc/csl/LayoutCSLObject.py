#!/usr/bin/env python
# encoding: utf-8
# 
# CiteProc-Py 
# Generates citations and references.
# 
# Copyright 2006, 2008 Johan Kool
# 
# This file is part of CiteProc-Py.
# 
# CiteProc-Py is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
# 
# CiteProc-Py is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU General Public License for more details.
# 
# You should have received a copy of the GNU General Public License
# along with CiteProc-Py.  If not, see <http://www.gnu.org/licenses/>.
# 
# Contact the author(s) if you wish to use CiteProc-Py under another
# license, e.g. for inclusion in (commercial) closed source applications.
# It is explicitly forbidden to use CiteProc-Py in such applications
# under the GPL license.
#

"""
LayoutCSLObject.py

Created by Johan Kool on 2008-02-02.
Copyright (c) 2008 Johan Kool. All rights reserved.
"""

import sys
import os
import unittest

# div {
#   layout = element cs:layout { formatting, delimiter, cs-element }
#   cs-element = (names | date | label | cs-text | cs-number | choose | group)+
# }



from FormattingCSLObject import FormattingCSLObject
from DelimiterCSLObject import DelimiterCSLObject
from ParentCSLObject import ParentCSLObject

class LayoutCSLObject(FormattingCSLObject, DelimiterCSLObject, ParentCSLObject):
    def __init__(self, attrs):
        super(LayoutCSLObject, self).__init__(attrs)
        # array_of_elements may contain only objects of class NamesCSLObject, DateCSLObject, LabelCSLObject, (subclasses of) TextCSLObject, NumberCSLObject, ChooseCSLObject, GroupCSLObject
    
    def formatted_text_for_citationgroup(self, citationGroup, references, options):
        formatted_textList = []
        for citation in citationGroup.citations:
            formatted_text = u""
            options["kind"] = "citation"
            options["position"] = citation.position  # FIXME "first" | "subsequent" | "ibid" | "ibid-with-locator"
            reference = citation.reference
            for child in self.children:
                formatted_text += child.formatted_text(reference, options)
            formatted_textList.append(formatted_text)
        text = self.group(formatted_textList)
        return self.format(text)
    
    def formatted_text_for_references(self, references, options):
        formatted_textList = []
        for reference in references:
            formatted_text = u""
            for child in self.children:
                formatted_text += child.formatted_text(reference, options)
            formatted_textList.append(formatted_text)
        text = "\n\n".join(formatted_textList)
        
        return self.format(text)
                
    def xml(self):
        return "<layout%s%s>\n%s\n</layout>\n" % ( FormattingCSLObject.xml(self), DelimiterCSLObject.xml(self), ParentCSLObject.xml(self))
    

class LayoutCSLObjectTests(unittest.TestCase):
    def setUp(self):
        pass


if __name__ == '__main__':
    unittest.main()